    ...
    <div class="dashboard-body">
      ...
      >
        ...
        <div class="room-stage">
          ...
          <div
            v-for="badge in roomBadges"
            :key="badge.key"
            class="device-badge"
            :class="badgeStatus(badge.rows)"
            :style="{ left: badge.left, top: badge.top }"
          >
            <header>{{ badge.title }}</header>
            <p v-for="row in badge.rows" :key="row[0]">
              <span>{{ row[1] }}</span>
              <strong>{{ value(row[0]) }}</strong>
              <small>{{ row[2] }}</small>
            </p>
          </div>
        </div>
...
const hasInspectionBasis = computed(() =>
  qcSnapshotAvailable.value || payload.value.environment_fallback === true || availableCount.value > 0)
const scoreDisplay = computed(() => hasInspectionBasis.value ? score.value : '—')
const scoreLabel = computed(() => hasInspectionBasis.value ? '巡检评分' : '数据待补')
...
const scoreTone = computed(() => !hasInspectionBasis.value ? 'pending' : (score.value >= 90 ? 'good' : (score.value >= 60 ? 'medium' : 'bad')))
...
const clearResultDescription = computed(() => hasInspectionBasis.value
...
const roomBadges = [
  { key: 'electric', title: '电流电压', left: '19.5%', top: '69%',
    rows: [
      ['IA', 'A相电流', 'A'], ['IB', 'B相电流', 'A'], ['IC', 'C相电流', 'A'],
      ['VA', 'A相电压', 'V'], ['VB', 'B相电压', 'V'], ['VC', 'C相电压', 'V']
    ] },
  { key: 'station-env', title: '站房温湿度', left: '63%', top: '52%',
    rows: [['StationTemp', '温度', '℃'], ['StationHum', '湿度', '%']] },
  { key: 'sampling', title: '采样总管', left: '61.5%', top: '61.8%',
    rows: [
      ['PipeTemp', '总管温度', '℃'], ['PipeHum', '总管湿度', '%'],
      ['SamplePipeSPress', '总管静压', 'Pa'], ['SampleFlow', '采样流量', 'L/min']
    ] }
]
const badgeStatus = rows => {
  const classes = rows.map(row => statusClass(row[0]))
  if (classes.includes('error')) return 'error'
  if (classes.includes('offline')) return 'offline'
  if (classes.includes('missing')) return 'missing'
  return 'normal'
}
...
.station-heading { display: grid; gap: 4px; }.station-heading .eyebrow { color: #5bc7ff; font-size: 10px; letter-spacing: 2px; }.station-heading strong { color: var(--bg-container); font-size: 22px; text-shadow: 0 0 12px rgba(71, 190, 255, .55); }.station-meta { display: flex; gap: 18px; color: #a8d7f8; font-size: 11px; }
.inspection-result { display: flex; align-items: center; gap: 16px; }.score-ring { display: grid; width: 62px; height: 62px; place-content: center; border: 3px solid; border-radius: 50%; background: rgba(1, 28, 59, .78); box-shadow: 0 0 0 6px rgba(80, 200, 120, .12), inset 0 0 16px rgba(80, 200, 120, .18); text-align: center; }.score-ring span { font-size: 24px; font-weight: 800; line-height: 25px; }.score-ring small { font-size: 9px; }.score-ring.good { border-color: #37e57b; color: #37e57b; }.score-ring.medium { border-color: #45b6ff; color: #45b6ff; }.score-ring.bad { border-color: #ff694b; color: #ff694b; }.score-ring.pending { border-color: var(--color-warning); color: var(--color-warning); box-shadow: 0 0 0 6px rgba(255, 189, 57, .1), inset 0 0 16px rgba(255, 189, 57, .12); }.result-copy { display: grid; gap: 8px; }.result-copy strong { color: var(--bg-container); font-size: 16px; }.result-copy em { color: #ff704d; font-size: 22px; font-style: normal; }.result-copy span { color: #8ecaf1; font-size: 10px; }
...
.section-title, .panel-title { position: relative; z-index: 2; display: flex; height: 32px; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(78, 173, 238, .42); color: #e8f7ff; font-size: 13px; font-weight: 700; }.section-title::before, .panel-title::before { width: 3px; height: 14px; margin-right: 8px; background: #1ac8ff; box-shadow: 0 0 9px #1ac8ff; content: ""; }.section-title > span, .panel-title > span { margin-right: auto; }.legend { display: flex; align-items: center; gap: 6px; color: #83b5d8; font-size: 10px; font-weight: 400; }.legend i { width: 7px; height: 7px; margin-left: 8px; border-radius: 50%; }.legend .normal { background: #20df70; box-shadow: 0 0 7px #20df70; }.legend .error { background: var(--color-danger); }.legend .offline { background: #ffb11b; }
...
.reading { position: absolute; display: flex; align-items: center; justify-content: center; gap: 2px; padding: 1px 2px; overflow: hidden; border: 1px solid rgba(255,255,255,.4); border-radius: 1px; background: rgba(0, 198, 73, .9); box-sizing: border-box; color: var(--bg-container); box-shadow: 0 0 8px rgba(0, 235, 104, .3); line-height: 1; white-space: nowrap; }.reading strong { font-size: 10px; }.reading small { font-size: 6px; }.reading.missing { background: rgba(78, 99, 119, .86); box-shadow: none; }.reading.error { background: rgba(226, 53, 46, .92); }.reading.offline { background: rgba(224, 145, 17, .92); }.pm10 { left: 30.8%; top: 51.8%; width: 9.4%; height: 4.8%; }.pm25 { left: 45.8%; top: 51.8%; width: 9.4%; height: 4.8%; }.pollutant { left: 34.2%; width: 5.8%; height: 3.4%; }.pollutant strong { font-size: 9px; }.no { top: 64.2%; }.so2 { top: 72%; }.o3 { top: 79%; }.co { top: 85.6%; }
.bottle-value { position: absolute; top: 77.5%; width: 4%; color: var(--bg-container); font-size: 8px; text-align: center; text-shadow: 0 0 6px #000; }.bottle-so2 { left: 58.2%; }.bottle-no { left: 62.3%; }.bottle-co { left: 66.3%; }.bottle-value.error { color: #ff5454; }.bottle-value.offline, .bottle-value.missing { color: #ffd05b; }
.device-badge { position: absolute; z-index: 3; display: grid; gap: 1px; min-width: 92px; padding: 3px 5px; border: 1px solid rgba(255,255,255,.4); border-radius: 2px; background: rgba(6, 34, 62, .84); box-shadow: 0 0 8px rgba(0, 20, 40, .42); line-height: 1.25; white-space: nowrap; }.device-badge header { color: #7fd3ff; font-size: 8px; font-weight: 700; }.device-badge p { display: flex; align-items: baseline; gap: 3px; margin: 0; }.device-badge span { color: #9fc2dc; font-size: 7px; }.device-badge strong { color: var(--bg-container); font-size: 9px; }.device-badge small { color: #7fa6c2; font-size: 6px; }.device-badge.error { border-color: #ff564f; }.device-badge.offline { border-color: #ffb11b; }.device-badge.missing { opacity: .6; }
.room-metrics { position: relative; z-index: 2; display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; padding-top: 8px; border-top: 1px solid rgba(70, 170, 235, .34); }.room-metrics article { display: grid; min-width: 0; align-content: center; gap: 3px; padding: 6px 9px; border: 1px solid rgba(45, 150, 216, .42); border-left: 3px solid #1edb73; border-radius: 3px; background: rgba(4, 48, 85, .78); }.room-metrics span { color: #6dc9f5; font-size: 8px; }.room-metrics strong { overflow: hidden; color: var(--bg-container); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.room-metrics small { overflow: hidden; color: #7ca6c1; font-size: 7px; text-overflow: ellipsis; white-space: nowrap; }
