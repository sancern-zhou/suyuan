<template>
  <section class="graph-toolbar">
    ...
    <label><input type="checkbox" :checked="neighborhoodMode" @change="$emit('neighborhood', $event.target.checked)" />局部邻域</label>
...
defineProps({
  ...
  showLabels: { type: Boolean, default: true }, neighborhoodMode: { type: Boolean, default: false }, includeHistory: { type: Boolean, default: false },
...
const emit = defineEmits(['search', 'entity-filter', 'relation-filter', 'labels', 'neighborhood', 'fit', 'layout', 'fullscreen', 'history', 'refresh'])
const query = ref('')
