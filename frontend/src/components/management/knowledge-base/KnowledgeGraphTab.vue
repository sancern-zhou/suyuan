...
<style scoped>
.knowledge-graph-tab { display: grid; gap: 12px; }.graph-workbench { display: flex; gap: 12px; align-items: stretch; }.graph-workbench > :first-child { flex: 1; min-width: 0; }.error { color: var(--color-danger); }.merge-notice { padding: 8px; background: #fff6df; border-radius: 6px; }
.graph-build-panel { display: grid; grid-template-columns: minmax(220px, 1fr) auto; gap: 8px 16px; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border-2); }.graph-build-panel p { margin: 4px 0 0; color: var(--text-2); font-size: 13px; }.graph-build-actions { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }.graph-build-actions select, .graph-build-actions button { min-height: 34px; }.graph-build-actions button { border: 1px solid #176b87; background: #176b87; color: white; padding: 0 12px; border-radius: 6px; cursor: pointer; }.graph-build-actions button.secondary { color: #344054; border-color: #d0d5dd; background: white; }.graph-build-actions button:disabled { opacity: .55; cursor: default; }.graph-build-progress { grid-column: 1 / -1; color: #475467; font-size: 13px; }
.knowledge-graph-tab:fullscreen { background: var(--bg-muted); padding: 14px; overflow: auto; }
