...
test('query dashboard layout keeps chat on the right and controls at bottom left', () => {
  ...
  assert.match(source, /\.chat-overlay\s*\{[\s\S]*?top:\s*16px;[\s\S]*?right:\s*18px;[\s\S]*?width:\s*min\(480px, calc\(100% - 36px\)\);/)
  assert.match(source, /\.dashboard-mode-selector\s*\{[\s\S]*?right:\s*18px;[\s\S]*?bottom:\s*16px;[\s\S]*?width:\s*min\(480px, calc\(100% - 36px\)\);[\s\S]*?flex-wrap:\s*nowrap;/)
