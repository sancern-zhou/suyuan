<template><div class="chart"><p v-if="loading">正在加载...</p><div v-else-if="error" class="error"><span>{{ error }}</span><button type="button" @click="load">重试</button></div><TaskReviewPanel v-else-if="spec?.type === 'task_review'" :review-id="spec.data.review_id" /><ChartPanel v-else-if="spec" ref="panelRef" :data="spec" /></div></template>
<script setup>
...
const getChartImage = async (options) => panelRef.value?.getChartImage ? panelRef.value.getChartImage(options) : null
