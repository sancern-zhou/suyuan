...
test('preview host uses the resource store and opaque content boundary', async () => {
  ...
  assert.match(source, /class="document-toolbar"/)
  assert.match(source, /class="document-picker"/)
  assert.match(source, /document-picker'/)
  assert.match(source, /compact/)
  assert.match(source, /\.preview-layout \{ position: relative;/)
  assert.doesNotMatch(source, /file_path|pdf_id|html_id|\/api\/file\//)
})

test('document preview actions expose a compact toolbar menu', async () => {
  ...
  assert.match(source, /\{ floating, compact \}/)
  assert.match(source, /resource-actions\.compact/)
