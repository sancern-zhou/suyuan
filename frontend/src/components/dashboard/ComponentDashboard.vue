<template>
  <div class="component-dashboard">
    ...
    <div class="pagination-controls" v-if="totalPages > 1">
      <div class="page-info">
        <span class="page-label">图表展示</span>
        <span class="page-number">第 {{ currentPage }} / {{ totalPages }} 页</span>
        <span class="total-count">(共 {{ sortedVisualizations.length }} 个)</span>
      </div>
    ...
    <div class="smart-grid">
      ...
      >
        <div class="card-content">
          ...
          <ChartPanel
            v-if="isChartType(item.viz.type)"
            :data="item.viz"
          />
          <DataTable
      ...
      <div v-if="enlargedChart" class="chart-modal-overlay" @click="closeModal">
        <div class="chart-modal" @click.stop>
          ...
          <div class="modal-body">
            <ChartPanel
              v-if="isChartType(enlargedChart.type)"
              :data="enlargedChart"
              customHeight="500px"
            />
            <DataTable
    ...
    <div class="expert-summary-bar" v-if="expertAnalysis">
      <div class="summary-header">
        ...
        <span class="summary-title">组分分析专家总结</span>
      </div>
...
const CHART_TYPES = ['chart', 'pie', 'bar', 'line', 'timeseries', 'radar', 'heatmap', 'scatter3d', 'surface3d']
...
const totalPages = computed(() => {
  return Math.ceil(sortedVisualizations.value.length / PAGE_SIZE) || 1
})
...
const expertAnalysis = computed(() => {
  ...
  // 从 expert_results 中提取组件专家数据（注意：后端存储在.analysis字段中）
  const componentResult = props.expertResults.expert_results.component?.analysis
  if (!componentResult) return null

  // 返回完整的分析信息
  return {
    summary: componentResult.summary || '',
    key_findings: componentResult.key_findings || [],
    confidence: componentResult.confidence || null,
    data_quality: componentResult.data_quality || null
  }
})
