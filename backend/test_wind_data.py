...
async def check_era5_wind_data():
    ...
    async with async_session() as session:
        ...
        # 7. 按网格点统计缺失情况
        print("\n按网格点统计:")
        grid_stats_result = await session.execute(
            select(
                ...
                ).label('wind_speed_null'),
                func.sum(
                    case(
                        (ERA5ReanalysisData.wind_direction_10m.is_(None), 1),
                        else_=0
                    )
                ).label('wind_direction_null')
            )
            .group_by(ERA5ReanalysisData.lat, ERA5ReanalysisData.lon)
            .order_by('total', 'lat', 'lon')
        )

        grid_stats = grid_stats_result.fetchall()
        for stat in grid_stats:
            lat, lon, total, ws_null, wd_null = stat
            ws_pct = ws_null / total * 100 if total > 0 else 0
            wd_pct = wd_null / total * 100 if total > 0 else 0
            print(f"  网格点 ({lat:.2f}, {lon:.2f}): 总计={total:6d} | "
                  f"风速NULL={ws_null:5d} ({ws_pct:5.1f}%) | "
                  f"风向NULL={wd_null:5d} ({wd_pct:5.1f}%)")
...
async def check_observed_wind_data():
    ...
    async with async_session() as session:
        ...
        # 5. 按站点统计
        print("\n按站点统计:")
        station_stats_result = await session.execute(
            select(
                ObservedWeatherData.station_id,
                func.count().label('total'),
                func.sum(
                    case(
                        (ObservedWeatherData.wind_speed_10m.is_(None), 1),
                        else_=0
                    )
                ).label('wind_speed_null'),
                func.sum(
                    case(
                        (ObservedWeatherData.wind_direction_10m.is_(None), 1),
                        else_=0
                    )
                ).label('wind_direction_null')
            )
            .group_by(ObservedWeatherData.station_id)
            .order_by('total', 'station_id')
            .limit(20)
        )

        station_stats = station_stats_result.fetchall()
        for stat in station_stats:
            station_id, total, ws_null, wd_null = stat
            ws_pct = ws_null / total * 100 if total > 0 else 0
            wd_pct = wd_null / total * 100 if total > 0 else 0
            print(f"  站点 {station_id}: 总计={total:6d} | "
                  f"风速NULL={ws_null:5d} ({ws_pct:5.1f}%) | "
                  f"风向NULL={wd_null:5d} ({wd_pct:5.1f}%)")
