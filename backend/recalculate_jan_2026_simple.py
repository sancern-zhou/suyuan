"""
重新计算2026年1月份省级统计数据（简化版）

直接计算current_month和annual_ytd，跳过monthly转换
...
async def recalculate_january_2026():
    """重新计算2026年1月份的省级统计数据（简化版）"""

    print("="*100)
    print("重新计算2026年1月份省级统计数据（简化版）")
    print("="*100)
    ...
    # 模拟2026年1月31日（1月最后一天）
    today = datetime(2026, 1, 31).date()

    print(f"\n模拟日期：{today.isoformat()}")
    print(f"目标：重新计算2026年1月的省级统计\n")

    try:
        # 步骤1：重新计算2026年1月的current_month数据
        print("步骤1：重新计算2026年1月的current_month数据")
        print("-"*100)
        await fetcher._calculate_and_store_current_month(today)
        print("✓ current_month计算完成\n")

        # 步骤2：重新计算2026年度累计数据（截至1月31日）
        print("步骤2：重新计算2026年度累计数据（截至1月31日）")
        print("-"*100)
...
async def verify_results():
    ...
    try:
        ...
        # 显示annual_ytd数据
        print("\n2. 年度累计（annual_ytd）- 2026年1月1日至1月31日")
        print("-"*100)
        if annual_ytd_data:
            print(f"{'省份':10s} {'PM2.5均值':>12s} {'城市数':>8s} {'数据天数':>10s} {'样本覆盖率':>12s}")
            print("-"*100)
        ...
        for province in ['广东', '新疆']:
            ...
            if row:
                ...
                print(f"  城市列表: {row.city_names[:100]}...")  # 只显示前100个字符
                ...
                if province == '广东':
                    expected_cities = 21
                    if row.city_count == expected_cities:
                        print(f"  ✓ 城市数量正确：{expected_cities}个（全省所有地级市）")
                    else:
                        print(f"  ✗ 城市数量异常：预期{expected_cities}，实际{row.city_count}")
                elif province == '新疆':
                    expected_cities = 24
                    if row.city_count == expected_cities:
                        print(f"  ✓ 城市数量正确：{expected_cities}个（全省所有地级市）")
