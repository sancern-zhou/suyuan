...
@pytest.mark.asyncio
async def test_plain_text_final_response_does_not_require_json(
    service,
    event_task,
):
    service.executor.agent_factory = lambda: PlainTextAgent()
    service.create_task(event_task)

    result = await service.publish_event(_event("plain-text-result"), wait=True)

    execution = service.execution_storage.get(result.execution_ids[0])
    assert execution.status.value == "success"
    assert execution.error_message is None
    assert execution.steps[0].agent_response.startswith("日报已广播")
