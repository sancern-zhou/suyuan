"""
更新province_statistics表中历史数据的旧标准综合指数
...
class ProvinceOldStandardIndexUpdater:
    """省级旧标准综合指数更新器"""
    ...
    def fetch_all_statistics(self) -> list:
        ...
        sql = """
        SELECT
            id, stat_date, stat_type, province_name,
        ...
        ORDER BY stat_date DESC, stat_type, province_name
        """
        ...
        try:
            ...
            logger.error(
                "fetch_province_statistics_failed",
                error=str(e)
            )
            raise
    ...
    def update_database(self, updates: list):
        ...
        """
        if not updates:
            logger.info("no_province_updates_needed")
            return
        ...
        try:
            ...
            logger.info(
                "province_database_update_completed",
                total_updated=len(updates)
            )

        except Exception as e:
            logger.error(
                "province_database_update_failed",
                error=str(e)
            )
            raise
    ...
    def calculate_and_update_rankings(self):
        ...
        try:
            ...
            for group in groups:
                ...
                # 获取该分组的所有记录
                records_sql = """
                SELECT id, province_name, comprehensive_index_old
                FROM province_statistics
                ...
                logger.info(
                    "province_ranking_updated",
                    stat_date=stat_date,
                    stat_type=stat_type,
                    provinces_count=len(records)
                )

            cursor.close()
            conn.close()

            logger.info("province_ranking_update_completed")

        except Exception as e:
            logger.error(
                "province_ranking_update_failed",
                error=str(e)
            )
            raise
    ...
    def run(self):
        """执行更新流程"""
        logger.info("province_update_started")
        ...
        if not statistics:
            logger.info("no_province_statistics_found")
            return
...
def main():
    """主函数"""
    updater = ProvinceOldStandardIndexUpdater()
    updater.run()
