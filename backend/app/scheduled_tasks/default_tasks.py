...
JIANGSU_FAULT_WORK_ORDER_REVIEW_PROMPT = (
    ...
    "仅做追溯。comment 面向人工快速研判，固定两段：第一段写明审核是否通过与处置意见；"
    "第二段用一句话说明核心原因；若建议剔除数据，另起一段仅列剔除污染物与时间段"
    "（精确到小时区间）。事实一致性、逻辑一致性、证据缺口与过程分析一律写入 "
    "checks 与 sections，不得塞进 comment。"
    "长期记忆和历史案例只用于形成待核验假设，不能替代本次证据；本次确认的可复用经验由任务历史学习沉淀，不回写固定 SOP。"
