...
8. 拥有两个或以上业务子节点的父节点必须在 style 中声明 `container=1`、`swimlane` 或 `group`；装饰徽标、标题等相对定位小元素不算业务子节点。
9. 容器内部至少保留 40px 外边距和 50px 行间通道；不要将主流程、回环和外部依赖共用同一条中线。
...
3. 无法避让的连线会保留原始路径，并通过 `routing_status`、`routing_issues` 和路由指标报告；少量降级可以继续生成，但必须经过截图验收。
4. 当 `edge_edge_crossing_count > 3`、`remaining_intersection_count > 0`，或降级连线超过总数 20% 时，质量报告应保留明确 warning；Agent 应结合截图决定是继续局部调整、拆图还是接受候选，不得把这些指标当作系统级阻断条件。
...
7. **分层/分级架构图连线规则：** 层级之间的连线只允许"层级容器 → 层级容器"（如泳道带、分组框之间），严禁在具体模块/岗位节点之间画跨层级连线。跨层级的管理、调度、指导等关系，通过层级连线的标注文字或节点内说明文字表达，不单独拉线。
8. **同层内上下级连线锚点规则：** 自上而下层级中，父节点连线必须从最近边出发（`exitX=0.5;exitY=1`，即底部中心），子节点从顶部进入（`entryX=0.5;entryY=0`）。禁止使用顶部出发（`exitY=0`）再绕回的锚点配置。
9. **折点通道规则：** 显式折点只允许放在两行节点之间的空白通道内，禁止折点或连线路径穿越、翻越任何节点本体。如果无法确定安全通道，应省略折点由系统自动规划。
