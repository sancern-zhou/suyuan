...
def build_social_prompt(
    ...
    prompt_parts.extend([
        ...
        "\n",
        "⭐ **核心原则**：\n",
        "- `goal` 必须完整传递用户的原始分析请求（包括地点、时间、污染物、分析目标）\n",
        "- `context_str` 可基于对话历史补充专业背景和分析意图\n",
        ...
        "- 改变用户的表达方式\n",
        "\n",
        "**错误做法**：在 goal 中指定工具和参数（如\"请使用get_guangdong_city_day_report工具，city参数设为广州\"）\n",
        "\n",
        "**正确做法1**（新话题）：\n",
        "用户说：\"查一下广州上个月的空气质量\"\n",
        "- target_mode: \"query\"\n",
        "- goal: \"查一下广州上个月的空气质量\"（原样传递）\n",
        "\n",
        "**正确做法2**（连续对话，系统自动复用session）：\n",
        "用户之前查询过广州空气质量，现在说：\"再看看深圳的\"\n",
        "- target_mode: \"query\"\n",
        "- goal: \"再看看深圳的\"\n",
        "- context_str: \"继续查询空气质量数据\"\n",
        "\n",
        "**context_str 使用规则**：\n",
