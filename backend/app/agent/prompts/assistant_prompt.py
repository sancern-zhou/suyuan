...
def build_assistant_prompt(available_tools: List[str], memory_context: Optional[str] = None, memory_file_path: Optional[str] = None) -> str:
    ...
    prompt_parts.extend([
        ...
        "## 文件交付与前端预览\n",
        "\n",
        "前端支持多种文件交付方式：工具结果可自动触发右侧预览/下载，也可以在未自动触发时通过系统支持的预览链接或下载链接交付文件。\n",
        "\n",
        "**你需要遵守的行为规则**：\n",
        "- 生成、修改或读取用户可下载文件时，优先使用会返回结构化文档结果的工具，让系统自动触发右侧预览和下载能力。\n",
        "- 如果工具结果已经触发右侧预览/下载，最终回复只需说明文件已生成/已更新、文件名和简要结果，并提示用户可在右侧面板查看或下载。\n",
        "- 如果文件已生成但没有自动触发前端预览/下载，不要只回复本地路径；应基于工具结果、已知文件路径或系统支持能力提供可点击的预览链接或下载链接。\n",
        "- 用户想查看、预览或打开内容时，优先提供 HTML 预览方式；用户想下载、保存或获取文件时，优先提供下载方式。\n",
        "- 如果使用 `execute_python` 直接生成 Office、Markdown、HTML、Notebook 等文件，代码必须在保存后打印清晰的保存路径，便于工具提取并交给前端处理。\n",
        "- 对用户上传的附件，使用用户消息或工具结果提供的文件路径读取和处理；不要假设上传文件位于其他目录。\n",
        "- 不要猜测不存在的链接；本地绝对路径只能作为辅助诊断信息，不能作为唯一交付方式。\n",
        "\n",
        "**图片生成与渲染**：\n",
        ...
        "- `goal` 原样传递用户问题，禁止转换、澄清或改写\n",
        "- `context_str` 补充隐含信息（地理范围、时间范围、查询指标）\n",
        "- 禁止指定工具名称、构造技术参数、时间转换、意图澄清\n",
        "\n",
        f"**专家模式调用规则**（来自 `{expert_agent_guide_path_str}`）：\n",
        "- `goal` 完整描述分析需求（地点、时间、污染物、分析目标）\n",
        "- `context_str` 补充专业背景（分析目的、技术细节、数据情况）\n",
