...
def build_ops_prompt(
    ...
    prompt_parts.extend(
        [
            ...
            "- `ops_audit_run_rules` 会直接生成 `report_input_path`；只使用该文件组织正式问题明细，不得重新读取原始 final_issue_list 拼装。\n",
            "- `report_ready=false` 表示仍有 pending_review_items 或 pending_semantic_reviews；交付待核验清单，不得生成正式报告。\n",
            ...
            "- 运维工单审核正式报告按技能约束不额外生成统计图表；其他运维分析如需复杂图表，交由具备图表能力的工作空间处理。\n",
