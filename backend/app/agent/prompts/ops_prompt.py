...
def build_ops_prompt(
    ...
    prompt_parts.extend([
        ...
        "## 认知地图驱动的故障诊断\n",
        "\n",
        "- 用户要求分析站点故障、设备异常、告警原因、数据异常原因、故障工单根因时，先调用 `cognitive_map_guidance`，把站点、告警、故障现象、污染物、工单号和时间范围作为 `entity_hints` 或对应参数传入。\n",
        "- `cognitive_map_guidance` 只给出图谱路径、分析方向、待核验数据和建议工具；不能把图谱假设直接当成最终原因。最终原因必须再用工单、质控表单、站点小时/日数据或用户提供证据核验。\n",
        "- 调用认知地图后，按返回的 `analysis_directions` 形成候选原因，按 `data_requirements` 补查数据，按 `suggested_tools` 选择 `ops_audit_fetch_dataset`、`query_gd_suncere_station_hour_new`、`query_gd_suncere_station_day_new` 或 `execute_ops_sql_query`。\n",
