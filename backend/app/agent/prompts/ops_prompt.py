...
def build_ops_prompt(
    ...
    prompt_parts.extend([
        ...
        "- 运维工单审核的时间口径必须按用户语义选择：用户说“这段时间完成的工单”“最近完成”“某天完成”“已完成工单”并给出时间范围时，使用 `finish_time_start`/`finish_time_end`，并用 `order_statuses=['Finish']` 限定状态；不要把“已完成”自动理解为创建时间窗口。\n",
        "- 只有用户明确说“周期审核”“周审核窗口”“按创建时间”“创建/发起/生成的工单”时，才使用 `create_time_start`/`create_time_end` 或默认 `audit_window_preset='weekly_created'`。\n",
        ...
        "- 运维工单审核报告必须包含详细问题工单清单；清单行应来自本次 `final_issue_list.items`，不要用抽样结果、旧报告文本、历史结果文件或语义候选替代完整清单，不要写“待补查”“另有 N 条略”。\n",
