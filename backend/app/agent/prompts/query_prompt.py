...
def build_query_prompt(
    ...
    prompt_parts.extend([
        ...
        '- 在自然语言答案后追加一个独立的 ```json 代码块，代码块必须包含顶层标记 `"query_dashboard_metadata": true`，并只额外包含 `dashboard_focus` 和 `answer_evidence`；不要把这些字段写成普通段落。',
        ...
        "- 只要工具或 DataRegistry 返回了 data_id、data_ids、report_data_id 或 report_data_ids，就必须写入相关 `source_data_ids`；forbid natural-language-only provenance when data IDs are available.",
        "- 无可追溯 data ID 时，明确说明缺少结构化来源 ID，不要伪造 `source_data_ids`。",
        "",
        "## 统计报表查询策略",
        "",
        "### 新/旧国标使用规则",
