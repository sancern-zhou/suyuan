...
def build_report_prompt(available_tools: List[str], memory_context: Optional[str] = None, memory_file_path: Optional[str] = None) -> str:
    ...
    prompt_parts.extend([
        ...
        "\n",
        "**默认政府报告格式**：生成DOCX时默认调用 `from app.services.report.government_docx_style import apply_government_report_style, add_government_title, add_government_heading, add_government_paragraph, add_government_table, format_government_table`。新建 `Document()` 后先执行 `apply_government_report_style(doc)`；标题、正文、标题层级和表格优先使用这些 helper。除非用户明确要求其他格式，否则不要自行发明字体、字号、行距和页边距。\n",
        "\n",
        "**默认格式要点**：标题居中、小标宋/宋体 fallback、二号；正文仿宋三号、首行缩进2字符、固定28磅行距；一级标题黑体三号，二级标题楷体三号，三级标题仿宋加粗三号；A4报告页边距上3.7cm、下3.5cm、左右2.8cm；表格居中、黑色单线边框、表头加粗。用户提出明确排版要求时，在此默认格式基础上局部覆盖。\n",
        "\n",
        "### 第六步：保存为新模板（可选）\n",
        "\n",
