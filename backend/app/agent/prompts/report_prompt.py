...
def build_report_prompt(available_tools: List[str], memory_context: Optional[str] = None, memory_file_path: Optional[str] = None) -> str:
    ...
    prompt_parts.extend([
        ...
        "报告 DAG 完成后必须检查返回的 `data.report_delivery`：确认综合结论、证据引用和报告包产物齐全；状态为 `completed_with_gaps` 时先补齐缺口或明确告知用户，不得把不完整的报告包当作最终交付。\n",
    ...
    prompt_parts.extend([
        ...
        "\n",
        "**路径A：找到类似的计划模板**\n",
        "1. 使用 `grep` 搜索 `backend/backend_data_registry/report_templates/` 目录\n",
        ...
        "\n",
        *city_pollutant_ranking_guidance,
        "**⚠️ 默认城市范围**：如果用户没有指定城市，按当前项目配置或工具默认范围查询；用户明确指定城市或区域时，以本次请求为准。\n",
        "\n",
        ...
        "\n",
        "**场景2：找到计划模板（推荐）**\n",
        "1. 用户：生成2024年1月区域空气质量通报\n",
        "2. Agent：使用 `grep` 搜索计划模板 → 找到\"区域空气质量月度通报.md\"\n",
        ...
        "## 工作原则\n",
        "\n",
        "1. **⚠️ 优先搜索计划模板**：用户提出需求后，优先使用 `grep` 搜索 `backend/backend_data_registry/report_templates/` 目录，看是否有类似的计划模板（根据用户需求搜索，不要根据用户上传的DOCX文件名搜索）\n",
