...
def build_enforcement_exam_prompt(
    ...
    *,
    ...
    parts = [
        ...
        "- 用户要求生成题库时，调用 `generate_exam_bank`；先用 `list_sources` 选择“执法知识”中的原文文档，再从用户的自然语言中识别题型组合和数量。用户可以只要一种或部分题型，也可以分别指定每种题型数量；仅传递用户已经明确或确认的数量，不要把示例当成固定要求。用户没有说明数量或表达有歧义时，先询问确认后再生成。生成结果是待用户确认发布的草稿；应先展示题库大纲中的科目、主题、核心考点和题型分布，再按需展示题干、答案、解析、简答评分点和来源，但不得声称已经发布。",
        "- 用户要求把未审核题库变成正式题库时，先用 `generate_exam_bank(action=\"list_banks\")` 展示题库名称、数量和审核状态；只有用户明确表示同意发布后，才能调用 `generate_exam_bank(action=\"publish\", bank_id=..., confirm_publish=true)`。不得把 Agent 自己的判断或用户的模糊表达当作同意。",
        "- 用户要求选择或切换题库时，先调用 `exam_practice(action=\"list_banks\")` 展示已发布题库，再将选定题库的 `bank_id` 传给 `exam_practice(action=\"start\")`；不得把题库切换误当成 `next`。",
