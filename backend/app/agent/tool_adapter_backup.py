...
TOOL_DEPENDENCIES = {
    # 分析工具依赖
    "calculate_pm_pmf": "需先调用 get_particulate_data 获取【水溶性离子小时粒度数据】(role=water-soluble)和【碳组分小时粒度数据】(role=carbon)，分别作为data_id和gas_data_id参数",
    ...
    "calculate_reconstruction": "需先调用 get_particulate_data 获取颗粒物组分数据（包含水溶性离子、碳组分、地壳元素等完整数据）",
    "calculate_carbon": "需先调用 get_particulate_data 获取碳组分数据（role=carbon，包含OC和EC字段）",
    "calculate_soluble": "需先调用 get_particulate_data 获取水溶性离子数据（role=water-soluble，包含SO4、NO3、NH4等字段）",
    "calculate_crustal": "需先调用 get_particulate_data 获取地壳元素数据（role=crustal，包含Ca、Fe、Al等元素）",
    "calculate_trace": "需先调用 get_particulate_data 获取微量元素数据（role=trace，包含重金属等微量元素）",
...
async def call_llm_tool(tool_name: str, context=None, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        if execution_context is not None:
            # 【修复】传递context作为关键字参数，而不是位置参数
            result = await tool.execute(context=execution_context, **exec_kwargs)
        else:
...
def _convert_to_standard_format(result: Dict[str, Any], tool_name: str, execution_time: float) -> Dict[str, Any]:
    ...
    try:
        ...
        summary = result.get("summary", f"✅ {tool_name} 执行完成")
...
def get_tool_summaries() -> str:
    ...
    优化策略：
    - 自然语言查询工具：直接展示规范化参数示例（免二次调用）
    - 复杂分析工具：标记需要详细参数说明（强制二次调用）

    节省token：从21.8K降低到~2K（节省90%）
    ...
    NLQ_TOOLS = {
        ...
        "get_particulate_data": {
            "desc": "颗粒物组分数据（（广东省的颗粒物组分站点级数据））",
            "example": "查询广州2025-01-15至2025-01-16的日度PM2.5组分数据"
        },
    ...
    SIMPLE_TOOLS = {
        ...
        "excel_processor": {
            "desc": "读取和编辑 Excel 工作簿（仅 Windows）",
            "example": 'file_path="D:\\\\docs\\\\data.xlsx", operation="list_sheets"'
        },
        "ppt_processor": {
            "desc": "读取和编辑 PowerPoint 演示文稿（仅 Windows）",
            "example": 'file_path="D:\\\\docs\\\\presentation.pptx", operation="list_slides"'
        }
    }
    ...
    for tool_data in global_tool_registry.get_all_tools():
        ...
        if tool.is_available():
            ...
            # 分类
            if any(kw in name for kw in ["get_", "search_", "universal_", "word_processor", "excel_processor", "ppt_processor"]):
            ...
            # 注意：bash 工具已在提示词中直接描述，不在摘要中重复显示
...
- 标记 ⚠️ 的工具：必须先查看详细参数说明（输出 args: null）
- 其他工具：有参数示例的可直接调用
