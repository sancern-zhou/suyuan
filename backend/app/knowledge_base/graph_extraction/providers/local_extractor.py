...
class LocalRuleBasedExtractorProvider:
    ...
    async def extract(
        ...
        knowledge_base_id = chunks[0].knowledge_base_id if chunks else "unknown"
        ...
        for chunk in chunks:
            for term, (entity_type, aliases) in self.TERM_CATALOG.items():
                ...
                evidence = evidence_by_chunk.setdefault(
                    chunk.chunk_id,
                    Evidence(
                        evidence_id=self._stable_id("ev", chunk.knowledge_base_id, chunk.chunk_id),
                ...
                if key not in entities_by_key:
                    entities_by_key[key] = CandidateEntity(
                        entity_id=self._stable_id("ent", chunk.knowledge_base_id, entity_type, name),
                        knowledge_base_id=chunk.knowledge_base_id,
    ...
    def extract_sync_from_text(
        ...
        async def _run() -> GraphExtractionResult:
