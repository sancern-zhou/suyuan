from __future__ import annotations

from app.knowledge_base.graph_extraction.providers.llamaindex_extractor import (
    LlamaIndexPropertyGraphExtractorProvider,
)
from app.knowledge_base.graph_extraction.providers.docx_parser import DocxParserProvider
from app.knowledge_base.graph_extraction.providers.local_extractor import LocalRuleBasedExtractorProvider
from app.knowledge_base.graph_extraction.providers.markitdown_parser import MarkItDownParserProvider
from app.knowledge_base.graph_extraction.providers.pdf_parser import PdfParserProvider
from app.knowledge_base.graph_extraction.providers.text_parser import TextParserProvider


def create_parser_provider(name: str | None = None):
    provider_name = (name or "text").strip().lower()
    if provider_name == "text":
        return TextParserProvider()
    if provider_name == "docx":
        return DocxParserProvider()
    if provider_name == "pdf":
        return PdfParserProvider()
    if provider_name == "markitdown":
        return MarkItDownParserProvider()
    raise ValueError(f"Unknown knowledge graph parser provider: {name}")


def create_extractor_provider(name: str | None = None, llm=None):
    provider_name = (name or "local").strip().lower()
    if provider_name == "local":
        return LocalRuleBasedExtractorProvider()
    if provider_name == "llamaindex":
        return LlamaIndexPropertyGraphExtractorProvider(llm=llm)
    raise ValueError(f"Unknown knowledge graph extractor provider: {name}")
