...
class MemoryStore:
    ...
    def __init__(
        self,
        user_id: Optional[str] = None,  # ✅ 新增：用户ID（格式：{channel}:{bot_account}:{sender_id}）
        ...
        # ✅ 用户专属工作空间
        self.workspace = self._init_workspace(workspace, user_id)
    ...
    def _init_workspace(self, workspace: Path, user_id: str) -> Path:
        """
        初始化用户专属工作空间
        ...
        Returns:
            用户专属工作空间路径
        """
        base = workspace or Path("backend_data_registry/social/memory")

        if user_id and user_id != "global":
            # 用户专属目录（格式：channel:bot_account:sender_id）
            # 文件系统将 : 替换为 _ 以避免路径问题
            safe_user_id = user_id.replace(":", "_")
            user_dir = base / safe_user_id
...
请只返回 JSON，不要有其他内容。"""
        ...
        try:
            ...
            if not entry or not update:
                logger.warning("memory_consolidation_empty_fields", user_id=self.user_id)
                return self._fail_or_raw_archive(messages)
            ...
            if not entry:
                logger.warning("memory_consolidation_empty_entry", user_id=self.user_id)
                return self._fail_or_raw_archive(messages)
...
class MemoryConsolidator:
    ...
    async def consolidate_messages(
        ...
        Returns:
            是否成功
        """
        store = ImprovedMemoryStore(user_id=user_id)
        return await store.consolidate_improved(messages, llm_call, model)
