...
def test_chart_route_restores_drawio_board_context_from_session_metadata():
    assert "drawio_board_context = request.board_context if request.mode == \"chart\" else None" in ROUTER_SOURCE
    assert "nonlocal actual_session_id, conversation_history, collected_data_ids, collected_visuals, latest_drawio_board, drawio_board_context, seen_visual_ids" in ROUTER_SOURCE
    assert "stored_drawio_board = session.metadata.get(\"drawio_board\")" in ROUTER_SOURCE
    assert "analyze_kwargs[\"board_context\"] = stored_drawio_board" in ROUTER_SOURCE
    assert "chart_board_context_restored_from_session_metadata" in ROUTER_SOURCE
