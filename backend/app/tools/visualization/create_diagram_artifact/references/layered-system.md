...
- 视觉采用传统 draw.io 分层架构图语言，画布保持白底，不采用网页 dashboard 卡片风格。
- 每层是一条横向浅色 band：左侧层名，右侧模块矩形；模块统一白底、细边框、轻圆角。
- 外部系统、人工服务、第三方系统使用右侧或边缘独立容器，与内部层分离。
- 字体与 Python 报告图表保持一致：优先 `FZXiaoBiaoSong-B05S`，其次 Noto Sans CJK / Droid Sans / PingFang。
...
- `shape`: 可选，写在模块 item 上；数据存储优先用 `database` 圆柱形，未知语义保持默认矩形。
...
- 分层架构图上层业务/支撑模块优先不用图标，因为类型常不完整且容易误导；传输、数据存储、感知等底部固定层可保留图标或 draw.io 常见组件形状。
  ...
  "layers": [
    ...
    },
    {
      ...
      "items": [
        ...
        {"label": "关系数据库", "shape": "database", "icon": "database"},
        {"label": "时序数据", "shape": "database", "icon": "timeseries"},
        {"label": "对象存储", "shape": "database", "icon": "object-storage"}
      ]
    }
  ],
  "edges": [
...
- 层标题的视觉权重高于普通模块。
- 模块节点等宽或等高，避免同层节点忽大忽小。
- 分层图默认采用压缩层高和放大节点文字的展示方式，优先保证放入方案/PPT 后仍能看清文字。
- 高密度层自动采用更紧凑的网格和间距；但这只是视觉保护，不能替代 Agent 的语义拆分。
- 层之间留白要大于同层模块间距，形成清晰分隔。
- 不同层使用低饱和浅色区分，不要彩虹配色，不要大面积高饱和色。
- 上层业务和支撑模块不要为了装饰添加图标；传统 draw.io 架构图优先靠文字、容器和连线表达结构。
- Agent 只需声明语义，不要在 label 中写“重点”“核心”来暗示样式。
...
- 不要使用网页组件语言：浮动卡片、强阴影、胶囊编号、渐变背景、hover 效果。
