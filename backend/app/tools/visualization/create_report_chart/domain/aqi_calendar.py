...
def _scaled_font_size(base_size: int, font_scale: FontScale = None) -> int:
...
class AQICalendarRenderer:
    ...
    def draw_matrix_heatmap(
        ...
        # 固定色块高度随字号同步缩放，避免放大文字后挤出单元格
        fixed_cell_height_pt = _scaled_font_size(12, font_scale)
        ...
        # 绘制日期（第一行）- 只显示有数据的日期
        date_font_size = _scaled_font_size(12, font_scale)
        ...
        for i, city in enumerate(cities):
            ...
            ax.text(x_left, y, city,
                   ha='right', va='center',
                   fontsize=_scaled_font_size(12, font_scale),  # 与网格高度一致
                   color='black',
    ...
    def add_color_legend(self, fig: plt.Figure, font_scale: FontScale = None) -> None:
        ...
        # 图例字体大小（使用已配置的中文字体）
        legend_font_size = _scaled_font_size(12, font_scale)
        legend_title_font_size = _scaled_font_size(14, font_scale)
    ...
    def render_calendar(
        ...
        fixed_cell_height_pt = _scaled_font_size(12, font_scale)  # 每个色块固定12点
        ...
        ax.set_title(title, fontsize=_scaled_font_size(24, font_scale), pad=20)

        logger.info("title_set", title=title)

        # 绘制矩阵热力图
        self.draw_matrix_heatmap(ax, city_data_map, year, month, pollutant, font_scale=font_scale)

        # 添加颜色图例
        self.add_color_legend(fig, font_scale=font_scale)
    ...
    def render_calendar_to_bytes(self, city_data_map, year, month, pollutant, font_scale: FontScale = None):
