...
from app.tools.visualization.font_sizing import (
    DEFAULT_WORD_IMAGE_WIDTH_IN,
    FontScale,
    OutputContext,
...
def generate_pollution_rose_contour(
    ...
    Args:
        ...
        font_scale: 字号缩放，可传 "small"、"normal"、"large"、"xlarge" 或 0.8-1.6 的数字；
                    默认normal
    ...
    figure_size = (10, 8)
    fonts = resolve_matplotlib_font_sizes(
        width_in=figure_size[0],
        height_in=figure_size[1],
        dpi=figure_dpi,
    ...
    ax.set_yticklabels([f'{s:.1f}' for s in tick_speeds], fontsize=fonts.tick_label)
    ax.set_ylabel('风速 (m/s)', fontsize=fonts.axis_label, labelpad=20)

    # 设置角度轴标签（方位）
    ax.set_xticks(np.radians([0, 90, 180, 270]))
    ax.set_xticklabels(['N', 'E', 'S', 'W'], fontsize=fonts.axis_label)
