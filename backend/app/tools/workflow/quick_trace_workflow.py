...
class QuickTraceWorkflow(WorkflowTool):
    ...
    description = """⚠️ 告警响应快速溯源（需要结构化参数）

【重要】此工具需要提供结构化参数，不支持自然语言查询。
如需使用自然语言查询，请使用任务清单驱动的流程（Agent会自动读取模板文件）。

专门用于污染高值告警场景的快速溯源分析，需要提供精确的告警信息：
