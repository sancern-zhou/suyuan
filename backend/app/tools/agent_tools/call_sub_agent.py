        ...
        **kwargs  # ✅ 捕获额外参数
        ...
        try:
            ...
            if target_mode == "expert":
                ...
                # 执行专家路由器
                pipeline_result = await sub_agent.execute_pipeline(
                    user_query=effective_goal,
                    precision="standard",  # 默认使用标准精度
                ...
                result_events = [{"type": "expert_result", "data": pipeline_result.to_dict()}]
                ...
                # 返回结果
                return {
                    "status": "success" if pipeline_result.status == "success" else "partial",
                    "success": pipeline_result.status == "success",
                    "result": pipeline_result.response,
                    ...
                    "summary": f"专家分析完成，状态：{pipeline_result.status}，参与专家：{', '.join(pipeline_result.selected_experts)}"
                }
