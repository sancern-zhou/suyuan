...
class SpatialInterpolationTool(LLMTool):
    def __init__(self) -> None:
        super().__init__(
            ...
            description=(
                ...
                "Outputs session-scoped grid/surface/contour data files for map analysis; use the surface file_path with visual_interaction interpolation-layer for user-visible map rendering. "
            ...
            function_schema={
                "name": "spatial_interpolation",
                "description": (
                    "浓度空间插值分析工具。输入会话数据 file_path、经纬度字段和值字段，"
                    "生成插值网格、插值渲染面和等值线数据文件，并登记到统一资源目录；本工具不生成静态图片。"
                    ...
                    "需要显示到地图时，优先使用返回的 surface file_path 调用 visual_interaction map-spec create interpolation-layer；"
                    "contours file_path 可用 visual_interaction map-spec create line-layer 作为可选等值线叠加。"
                ...
                "parameters": {
                    ...
                    "properties": {
                        "spec": {
                            "type": "object",
                            "description": (
                                "空间插值 spec。字段包括 file_path, lon, lat, value, pollutant, unit, method, "
    ...
    async def execute(self, context, spec: dict[str, Any], **_: Any) -> dict[str, Any]:
        return execute_interpolation(spec, context)
