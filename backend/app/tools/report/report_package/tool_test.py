...
def test_normalize_static_qmd_converts_ascii_quotes_around_chinese_text():
    qmd = '2. **强化扬尘管控**：春季施工高峰期严格落实"六个百分百"要求\n'

    normalized = _normalize_static_qmd(qmd)

    assert '"六个百分百"' not in normalized
    assert "落实“六个百分百”要求" in normalized
