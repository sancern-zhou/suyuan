...
class PPTWin32LLMTool(LLMTool):
    ...
    def _to_udf_v2_format(
        ...
        standard_result = {
            ...
            "summary": result.get("summary", f"PowerPoint {operation} 操作完成")
        }
        ...
        if success:
            ...
            if operation == "stats":
                ...
                standard_result["data"] = result.get("slides", [])
            elif operation == "read":
                standard_result["data"] = result.get("text", "")
            elif operation == "search_and_replace":
                standard_result["data"] = {
                    "replacements": result.get("replacements", 0),
                    "matches": result.get("matches", []),
                    "output_file": result.get("output_file", file_path)
                }
