...
""",
        ...
        **kwargs
        ...
        try:
            ...
            if operation == "read":
                ...
                formatted_result = self._to_udf_v2_format(result, path, operation)
                # ✅ 每次操作后关闭 Word 实例
                word.close_app()
    ...
    async def _read_with_pagination(
        ...
        if full_result.get("status") != "success":
            return self._to_udf_v2_format(full_result, path, "read")
    ...
    def _to_udf_v2_format(
        ...
        standard_result = {
            ...
            "summary": result.get("summary", f"Word {operation} 操作完成")
        }
        ...
        if success:
            ...
            if operation == "stats":
                ...
                # ✅ 简洁的summary，只显示表格尺寸信息
                table_info_list = []
                for i, table in enumerate(tables_data):
                    rows = table.get("rows", 0)
                    cols = table.get("cols", 0)
                    table_info_list.append(
                        f"表格{i+1}: {rows}行×{cols}列"
                    )
                standard_result["summary"] = f"读取了 {len(tables_data)} 个表格\n" + "\n".join(table_info_list)
            elif operation == "read":
                # ✅ 返回完整的读取数据
                standard_result["data"] = {
                    k: v for k, v in result.items()
                    if k not in ["status", "summary"]  # 排除已处理的字段
                }
            elif operation == "insert":
                # ✅ 返回insert操作的完整结果
                standard_result["data"] = {
                ...
                standard_result["data"] = {
                    ...
                    "output_file": result.get("output_file", path)
                }
