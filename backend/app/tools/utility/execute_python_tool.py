    ...
    def get_function_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": (
                ...
                "ax.set_ylabel('纵轴', fontproperties=chinese_font)\n"
                "```\n\n"
                "【⚠️ 化学式下标显示（重要）】matplotlib中不要使用Unicode下标字符（如O₃、PM₂.₅、NO₂），因为中文字体不包含这些字形，会显示为方框。\n"
                "必须使用matplotlib的LaTeX mathtext语法：\n"
                "```python\n"
                "# ❌ 错误：Unicode下标（会显示为方框）\n"
                "ax.set_title('O₃浓度变化', fontproperties=chinese_font)  # ₃显示异常\n"
                "ax.set_ylabel('PM₂.₅ (μg/m³)', fontproperties=chinese_font)  # ₂₅显示异常\n\n"
                "# ✅ 正确：LaTeX mathtext语法\n"
                "ax.set_title(r'O$_3$浓度变化', fontproperties=chinese_font)\n"
                "ax.set_ylabel(r'PM$_{2.5}$ ($\\mu$g/m$^3$)', fontproperties=chinese_font)\n"
                "ax.set_xlabel(r'NO$_2$浓度', fontproperties=chinese_font)\n"
                "ax.set_ylabel(r'SO$_2$ ($\\mu$g/m$^3$)', fontproperties=chinese_font)\n"
                "```\n"
                "常用化学式LaTeX写法：O$_3$ / NO$_2$ / SO$_2$ / CO / PM$_{2.5}$ / PM$_{10}$\n\n"
                "文件路径（重要）：\n"
