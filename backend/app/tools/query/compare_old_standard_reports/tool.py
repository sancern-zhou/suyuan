...
class CompareOldStandardReportsTool(LLMTool):
    ...
    def __init__(self):
        function_schema = {
            ...
            "description": """对比两个时间段基于 HJ 633-2013 旧标准的空气质量统计报表。
...
- 全省汇总统计规则同 query_old_standard_report
...
- enable_sand_deduction: 是否启用扣沙处理（默认true）
    ...
    async def execute(self, context: ExecutionContext, **kwargs) -> Dict[str, Any]:
        ...
        if len(cities) == 1:
            city = cities[0]
            summary_text = f"{city} 旧标准报表对比分析完成（{query_period['start_date']}至{query_period['end_date']} vs {comparison_period['start_date']}至{comparison_period['end_date']}，数据为审核实况） | 无原始数据 data_id，统计汇总指标已完整展示在 result 字段中"
        else:
            summary_text = f"多城市旧标准报表对比分析完成（{query_period['start_date']}至{query_period['end_date']} vs {comparison_period['start_date']}至{comparison_period['end_date']}，共{len(cities)}个城市，数据为审核实况） | 无原始数据 data_id，统计汇总指标已完整展示在 result 字段中"
    ...
    def _calculate_comparison(
        ...
        if len(current_stats) > 1:
            ...
            city_only_current = {k: v for k, v in current_stats.items() if k != "regional_stats"}
            city_only_comparison = {k: v for k, v in comparison_stats.items() if k != "regional_stats"}
