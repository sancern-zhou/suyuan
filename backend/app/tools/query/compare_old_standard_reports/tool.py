...
class CompareOldStandardReportsTool(LLMTool):
    """旧标准报表对比分析工具"""
    ...
    def __init__(self):
        function_schema = {
            ...
            "description": (
                "【第一优先级】旧标准同比/环比/双时段对比工具，基于HJ 633-2013。"
        ...
        super().__init__(
            name="compare_old_standard_reports",
            description="Compare old standard reports between two time periods",
    ...
    async def execute(self, context: ExecutionContext, **kwargs) -> Dict[str, Any]:
        ...
        if isinstance(result.get("metadata"), dict):
            result["metadata"]["generator"] = "compare_old_standard_reports"
        ...
        current_task = execute_query_old_standard_report(
            cities=cities,
        ...
        comparison_task = execute_query_old_standard_report(
            cities=cities,
        ...
        if len(cities) == 1:
            city = cities[0]
            summary_text = f"{city} 旧标准报表对比分析完成（{query_period['start_date']}至{query_period['end_date']} vs {comparison_period['start_date']}至{comparison_period['end_date']}，数据为审核实况） | 统计汇总指标已完整展示在 result 字段中"
        else:
            summary_text = f"多城市旧标准报表对比分析完成（{query_period['start_date']}至{query_period['end_date']} vs {comparison_period['start_date']}至{comparison_period['end_date']}，共{len(cities)}个城市，数据为审核实况） | 统计汇总指标已完整展示在 result 字段中"
        ...
        result = {
            ...
            "metadata": {
                "schema_version": "v2.0",
                "generator": "compare_old_standard_reports",
        ...
        report_data_id = save_report_data_package(
            context=context,
            tool_name="compare_old_standard_reports",
            query={
                "cities": cities,
                "query_period": query_period,
                "comparison_period": comparison_period,
            ...
            extra_views={
                "province": comparison_result_data.get("province_wide") if isinstance(comparison_result_data, dict) else None,
            },
            exclude_primary_keys={"province_wide"},
            package_kind="old_standard_report_comparison",
        )
        attach_report_data_id(result, report_data_id, summary_label="旧标准对比报表")

        logger.info(
            "compare_old_standard_reports_completed",
    ...
    def _calculate_comparison(
        ...
        if len(current_stats) > 1:
            ...
            # 过滤掉 regional_stats 键，避免重复计算（query_old_standard_report 已将其加入 result）
            city_only_current = {
