...
- 支持新旧标准综合指数查询（city_168_statistics、province_statistics表）
...
【可用数据表】
- city_168_statistics: 168城市空气质量统计（支持新旧标准综合指数查询，stat_type: monthly/annual_ytd/current_month，数据周期2024-01至今，⚠️ 城市名不带'市'后缀）
- province_statistics: 省级空气质量统计（支持新旧标准综合指数查询，stat_type: monthly/annual_ytd/current_month，数据周期2024-01至今，⚠️ 省份名不带'省'后缀）
...
【新旧标准综合指数查询示例】
- 查询新标准月报数据：WHERE stat_type='monthly' AND standard_type='new'
- 查询旧标准月报数据：WHERE stat_type='monthly' AND standard_type='old'
- 查询年初至今数据：WHERE stat_type='annual_ytd'
- 查询当月数据：WHERE stat_type='current_month'
        """.strip(),
        "parameters": {
            "type": "object",
            "properties": {
