...
@pytest.mark.asyncio
async def test_present_artifact_renders_unmapped_qmd_as_report_preview(tmp_path, monkeypatch):
    report_root = tmp_path / "reports"
    source_dir = tmp_path / "source"
    source_qmd = source_dir / "original.qmd"
    source_dir.mkdir(parents=True)
    ...
    renderer._run_quarto = fake_run_quarto
    monkeypatch.setattr(report_preview_refresh, "quarto_report_renderer", renderer)
    monkeypatch.setattr(present_artifact_tool, "refresh_report_preview_for_qmd_path", lambda path: None)
    monkeypatch.setattr(
    ...
    assert result["data"]["html_preview"]["html_url"].startswith("/api/reports/source_qmd_")
    assert "markdown_preview" not in result["data"]
