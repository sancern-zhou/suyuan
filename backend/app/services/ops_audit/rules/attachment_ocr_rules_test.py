...
def _enable_flow_visual_rules(monkeypatch, *rule_ids):
    monkeypatch.setattr(
        attachment_ocr_rules,
        "load_semantic_review_profiles",
        lambda: {"flow_visual_enabled_rule_ids": list(rule_ids)},
    )
...
def test_default_visual_allowlist_skips_pm_temp_pressure_without_calling_model(monkeypatch):
...
def test_default_visual_allowlist_suppresses_gas_display_mismatch(monkeypatch):
    def fake_extract_attachment_json(source, *, provider, task, prompt):
        return {
            "status": "success",
            "data": {
...
def test_pm_membrane_prompt_targets_mass_factor_not_membrane_label_value(monkeypatch):
    _enable_flow_visual_rules(monkeypatch, "ATTACHMENT_PM_MEMBRANE_VALUE_MISMATCH")
...
def test_pm_temp_pressure_visual_rule_marks_mismatch_as_visual_review_candidate(monkeypatch):
    _enable_flow_visual_rules(monkeypatch, "ATTACHMENT_PM_TEMP_PRESSURE_VALUE_MISMATCH")
    ...
    def fake_extract_attachment_json(source, *, provider, task, prompt):
        return {
            ...
            "data": {
                ...
                "values": {
                    "PM25": {
                        "temperature_display": 21.1,
                        "temperature_standard": None,
                        "pressure_display": 99.8,
                        "pressure_standard": None,
                    }
                },
                "confidence": 0.93,
                "reason": "PM2.5温湿度气压仪器示值照片",
            },
        }
    ...
    evidence = json.loads(issues[0].evidence)
    assert evidence["needs_visual_review"] is True
    assert "高置信度" in evidence["promotion_policy"]
