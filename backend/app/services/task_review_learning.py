"""Durable human-feedback consumption for all task reviews."""
...
async def _consume_feedback(review_id, task, storage=None):
    ...
    def save_review(value):
        with review_lock(review_id):
            latest = load_review(review_id)
            for candidate in [latest, *latest.get('history', [])]:
                if (candidate.get('human_feedback') or {}).get('feedback_id') == value['human_feedback']['feedback_id']:
                    candidate['human_feedback'] = value['human_feedback']
                    persist(latest)
                    break
    ...
    with storage._lock():
        ...
        if not any(item.get('feedback_id') == feedback_id for item in cases):
            ...
            case = {
                ...
                'summary': f"事项 {feedback['subject_id']} 人工结论 {feedback['human']['decision']}："
                           + (feedback['comment'] or '确认 AI 建议'),
