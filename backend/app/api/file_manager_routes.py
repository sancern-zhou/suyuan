...
async def list_directory(path: str = Query("", description="目录路径，相对于/tmp")):
    ...
    try:
        ...
        # 清理路径参数（移除可能的前缀如 "[object PointerEvent]"）
        if path and "[" in path:
            raise HTTPException(status_code=400, detail="无效的路径参数")
        ...
        if relative_path:
            ...
            try:
                parent_relative = parent.relative_to(ALLOWED_ROOT)
                parent_path_str = str(parent_relative)
                # 如果父目录是 ALLOWED_ROOT，返回空字符串表示根目录
                parent_path = "" if parent_path_str == "." else parent_path_str
            except ValueError as e:
                logger.error(f"Failed to get parent relative path: {e}")
        ...
        try:
            for entry in target_path.iterdir():
                try:
                    ...
                    logger.error(f"Failed to get entry relative path: {e}")
                    continue
