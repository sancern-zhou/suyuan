...
def test_drawio_restore_surfaces_version_store_failures_instead_of_returning_empty_success():
    drawio_method = SESSION_ROUTES.split('@router.get("/{session_id}/drawio-board")', 1)[1].split(
        "def _sanitize_floats", 1
    )[0]
    assert "board_version_restore_failed" in drawio_method
    assert "status_code=503" in drawio_method
    assert "status_code=409" in drawio_method
