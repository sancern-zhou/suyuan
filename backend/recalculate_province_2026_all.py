"""
重新计算2026年省级统计数据（修正数据范围错误）

问题：之前的计算只统计了168重点城市，而非全国所有地级市
解决：使用 citycode 与 bsd_city 表关联，查询全国所有城市数据进行统计

功能：
1. 清空2026年错误的省级统计数据
2. 重新计算2026年1月至指定月份的省级统计数据
3. 包含月度统计(monthly)、当月累计(current_month)、年度累计(annual_ytd)

关键改进：
- 使用 COLLATE Chinese_PRC_CI_AS 确保中文正确匹配
- 通过 citycode 关联 bsd_city 表，而非城市名称
- 查询时直接获取省份信息，避免二次映射

用法：
    python recalculate_province_2026_all.py [月份]

示例：
    python recalculate_province_2026_all.py 4    # 重算1-4月
    python recalculate_province_2026_all.py       # 默认重算1-4月
"""
import sys
import asyncio
...
def clear_2026_province_statistics():
    ...
    try:
        ...
        conn.commit()
        print(f"  ✓ 已删除2026年省级统计数据：{deleted_count} 条记录")
...
def verify_results():
    ...
    try:
        ...
        # 查询每个省份的城市名单（2026-01 monthly）
        print("\n各省份城市名单（2026-01 monthly，按城市数量排序）：")
        print("-" * 80)

        cursor.execute("""
            SELECT province_name, city_count, city_names
            FROM province_statistics_new_standard
            WHERE stat_date = '2026-01' AND stat_type = 'monthly'
        ...
        # 查询CityDayAQIPublishHistory表中实际有数据的城市总数
        print("\n" + "-" * 80)
        print("CityDayAQIPublishHistory表中2026-01有数据的城市统计：")
        print("-" * 80)

        cursor.execute("""
            SELECT COUNT(DISTINCT CityCode) as total_cities
            FROM CityDayAQIPublishHistory
            WHERE TimePoint >= '2026-01-01' AND TimePoint <= '2026-01-31'
        """)
        row = cursor.fetchone()
        print(f"  总城市数：{row.total_cities}")

        # 按省份统计
        cursor.execute("""
            SELECT
                p.name as province_name,
                COUNT(DISTINCT r.CityCode) as city_count
            FROM CityDayAQIPublishHistory r
            JOIN bsd_city c ON CAST(r.CityCode AS nvarchar(50)) COLLATE Chinese_PRC_CI_AS = c.code COLLATE Chinese_PRC_CI_AS
            JOIN bsd_city p ON c.parentid = p.code AND p.level = '1'
            WHERE r.TimePoint >= '2026-01-01' AND r.TimePoint <= '2026-01-31'
              AND c.level = '2'
            GROUP BY p.name
        ...
        rows = cursor.fetchall()
        for row in rows:
            print(f"    {row.province_name}: {row.city_count}个城市")
...
async def main():
    """主函数"""
    # 获取要计算的月份（默认4月）
    end_month = int(sys.argv[1]) if len(sys.argv) > 1 else 4
    end_month = max(1, min(12, end_month))  # 限制在1-12范围内

    print("="*80)
    print("重新计算2026年省级统计数据（修正数据范围错误）")
    print("="*80)
    print(f"\n计算范围：2026年1月至{end_month}月")
    print("\n修正内容：")
    print("  - 之前：只统计168重点城市")
    print("  - 现在：统计全国所有地级市、州、盟")
    ...
    try:
        ...
        for month in range(1, end_month + 1):
            ...
            # 获取该月最后一天
            if month == 12:
                last_day = date(2027, 1, 1) - timedelta(days=1)
            else:
                last_day = date(2026, month + 1, 1) - timedelta(days=1)

            # 计算当月累计
            await calculate_province_current_month(2026, month, last_day.day)
        ...
        await calculate_province_annual_ytd(2026, last_day.month, last_day.day)
