...
-- 4. 为新字段创建索引
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_city_168_rank_new_limit_old_algo' AND object_id = OBJECT_ID('city_168_statistics'))
BEGIN
    CREATE INDEX idx_city_168_rank_new_limit_old_algo ON city_168_statistics(comprehensive_index_rank_new_limit_old_algo);
    PRINT 'Index idx_city_168_rank_new_limit_old_algo created successfully.';
END
GO

IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_city_168_rank_old_limit_new_algo' AND object_id = OBJECT_ID('city_168_statistics'))
BEGIN
    CREATE INDEX idx_city_168_rank_old_limit_new_algo ON city_168_statistics(comprehensive_index_rank_old_limit_new_algo);
    PRINT 'Index idx_city_168_rank_old_limit_new_algo created successfully.';
END
GO
