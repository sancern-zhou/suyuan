...
## 概述
用于在运城市小时盯守告警触发后，基于证据目录中的告警 JSON、上下文资产、气象和本地后向轨迹分析结果生成提示性溯源分析报告，并在同一证据目录导出 Word 文档和微信摘要。**本技能为带审核技能**：报告和摘要生成后，必须经人工审核确认后方可广播推送，未经审核不得直接发送。
...
9. 助手模式确认 Word 文件存在后，返回事件任务服务要求的广播正文和 Word 附件路径，**状态标记为 `pending_review`（待审核）**。
10. 事件任务服务将报告和摘要推送至审核人（配置的微信用户），审核人可回复"通过"或提出修改意见。
11. 审核通过后，事件任务服务将状态更新为 `reviewed`，广播服务正式推送给配置的微信用户并写入各自的社交主会话；审核不通过时，根据修改意见重新生成后再次提交审核。
...
11. 生成 500 字以内微信摘要。
12. 返回“助手模式返回要求”中的事件广播 JSON，由事件任务服务先推送给审核人（状态为 `pending_review`），而非直接广播给最终用户。
13. **人工审核环节**：
    - 审核人收到报告（Word 附件）和微信摘要后进行人工审核。
    - 审核通过：审核人回复"通过"，事件任务服务将状态更新为 `reviewed`，广播服务正式推送给所有配置的微信用户并写入各自的社交主会话。
    - 审核不通过：审核人提出修改意见，助手模式根据意见修改报告后重新提交审核，循环直至通过。
14. **未经审核通过的报告不得直接广播推送给最终用户。**
