...
5. 同步调用气象 expert 子 Agent，要求输出 `weather_draft_path`。
6. 同步调用常规 expert 子 Agent，要求输出 `routine_draft_path`。
7. 读取两个专家草稿，检查是否存在越界结论、AQI 口径错误、缺失资产未说明、驻场建议不可执行等问题。
8. 生成 `report_qmd_path`，并确保报告采用业务版报告结构，包含“本次告警概览、关键变化看板、天气与传输影响、可能影响因素、现场建议、需要补充确认的信息、附件与证据清单”。
...
主 Agent 调用专家子 Agent 时，必须把对应资产路径和分析任务写进 prompt；不得只笼统说“读取 manifest”。
...
| 后向轨迹 | `trajectory_analysis.json`、`trajectory.png` | 气象分析专家主责 | 判断潜在输送方向；只能作为传输提示。 |
| 风场图 | `wind_field.png` | 气象分析专家主责 | 判断告警时段风向风速和输送条件。 |
| 能见度、降水、雷达 | `visibility.png`、`rainfall_24h.png`、`radar_mosaic.png`、`radar_composite_reflectivity_003.png` | 气象分析专家主责 | 判断湿清除、降水回波、低能见度或天气过程影响。 |
| 水汽和降水预报 | `precipitable_water_000.png`、`precipitation_forecast*.png`、`hourly_precipitation_forecast.png` | 气象分析专家主责 | 判断未来降水清除和天气变化趋势。 |
| 风和温度预报 | `wind_forecast_*.png`、`national_tmax_forecast.png`、`national_tmin_forecast.png` | 气象分析专家主责 | 判断未来扩散、输送和光化学生成条件。 |
| 卫星火点 | `fire_hotspots_summary.json`、`fire_hotspots_map.png`、`fire_hotspots.json` | 常规分析专家主责，气象专家辅助判断输送方向 | 优先读取摘要和分布图，完整明细只作附件核查；作为周边生物质燃烧、露天焚烧或热异常线索，不得确认具体污染源。 |
| 未来气象预报 | `forecast_meteorology.json` | 气象分析专家主责 | 判断后续 1-5 天气象变化和风险持续性。 |
| 空气质量预报 | `air_quality_5day_forecast.json` | 常规分析专家主责 | 判断后续空气质量风险和是否需要持续盯守。 |
...
1. 气象分析专家子 Agent
   ...
   - 负责回答：
     ...
     - 后向轨迹和风场是否支持区域传输提示，来自哪个方向，证据强弱如何。
...
2. 常规分析专家子 Agent
   ...
   - 按“专家资产分工”读取告警 JSON、目标城市小时数据、周边城市小时数据、卫星火点摘要与分布图、空气质量预报；可引用气象专家草稿或气象资产，但不主导气象机制判断。
   - 负责回答：
     ...
     - 报告中“关键变化看板、可能影响因素、现场建议、需要补充确认的信息”应写哪些业务要点。
...
2. 关键变化看板
   - 用表格展示目标城市告警前后小时浓度变化、首要污染物或主要贡献污染物、AQI 变化。
   - 用表格或要点展示周边城市是否同步变化。
   - 不堆原始数据，只保留业务人员判断趋势所需的关键数值。

3. 天气与传输影响
   - 用业务语言说明高温、湿度、风向风速、降水、边界层或输送条件对污染变化的影响。
   - 必须插入本次抓取到的气象和传输图片，并在每张图下写 1-2 句图注，说明“图上看到了什么、对现场判断有什么帮助”。
   - 不能只罗列图片文件名。
...
- 图片使用相对路径引用。
- 如果 manifest 中列出的图片资产存在，必须插入报告正文；至少插入 1 张图片。
- 图片优先级为：`trajectory.png`、`wind_field.png`、`fire_hotspots_map.png`、`visibility.png`、`rainfall_24h.png`、`radar_mosaic.png`、`radar_composite_reflectivity_003.png`、`precipitable_water_000.png`、`precipitation_forecast.png`、`precipitation_forecast_024.png`、`precipitation_forecast_048.png`、`precipitation_forecast_072.png`、`hourly_precipitation_forecast.png`、`wind_forecast_024.png`、`wind_forecast_048.png`、`wind_forecast_072.png`、`national_tmax_forecast.png`、`national_tmin_forecast.png`、其他已抓取图片。
- 每张图片必须配业务化图注，说明图片含义和现场判断价值。
- 不存在的图片不得占位；必须在“需要补充确认的信息”中说明未能展示该图及影响。
- 表格使用 Markdown 表格。
- 不使用需要联网加载的资源。
