    ...
    if "data" in response:
        ...
        if "result" in data_obj:
            ...
            if len(records) > 0:
                ...
                # Remove Unicode chars for safe output
                keys_safe = [str(k).encode('ascii', 'ignore').decode('ascii') for k in first_record.keys()]
                print(f"\nFirst record fields (safe):")
                print(f"  Keys: {keys_safe}")
                print(f"  Keys (raw): {list(first_record.keys())}")
