...
示例：
- recalc_excel(file_path="report.xlsx")
- recalc_excel(file_path="data.xlsx", timeout=60)
        ...
        **kwargs
        ...
        try:
            ...
            try:
                ...
                logger.info("recalc_timeout", file=str(file_path))
                # Timeout 通常表示成功（LibreOffice 已完成）

            # 4. 扫描公式错误
            error_scan_result = self._scan_formula_errors(file_path)
            ...
            logger.error("recalc_failed", file=file_path, error=str(e))
            return {
                "success": False,
                "data": {"error": str(e)},
