...
将江西省噪声平台查询能力修复为可注册、可异步执行、可配置并且只在江西项目启用的 LLM 工具。当前交付范围包括站点小时值、站点日均值和城市小时值；城市日均接口因服务端地址尚未确认，不对外暴露，也不在本次修复中猜测或探查。
...
不公开 `query_city_day_data`，API 端点表中也不声明城市日均接口。
