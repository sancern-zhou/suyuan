# Freeform Editable Diagram Design

## 背景

当前架构图生成主要通过 `create_diagram_artifact` 的模板化 HTML 渲染实现。该方式适合稳定预览、报告嵌入和 Word A4 静态图片输出，但对复杂架构图不够灵活：Agent 需要先把内容压进 `layers/groups/items` 或 `steps/edges`，导致跨层依赖、混合部署、局部放大、旁路链路和自定义拓扑表达受限。同时，当前产物以 HTML/PNG 为主，用户无法像 Visio 或 diagrams.net 一样继续编辑图形。

本设计目标是增加自由画布能力，让 Agent 能直接生成和编辑可编辑架构图，同时继续提供可预览图片。

## 目标

- Agent 可以不受分层模板限制，按画布图元自由生成架构图。
...
- `freeform`：新增自由画布模式，Agent 直接传入画布元素，由工具生成 `.drawio` 主文件并导出预览图片。

不新增面向 Agent 的新工具名。这样 Agent 仍只需要记住一个图表工具，工具内部根据模式选择渲染器，前端 artifact 返回结构也能复用。

## 自由画布输入模型

`freeform` 模式使用接近 draw.io 画布的结构化模型：
...
未知类型降级为 `rounded_rect`，并在 metadata 中返回 warning。
...
连线可配置 `start_arrow`、`end_arrow`、`dashed`、`stroke`、`stroke_width`、`waypoints` 和短标签。`from/to` 优先引用 shape ID；无法引用时允许使用绝对坐标端点。

## 生成流程

1. Agent 根据用户需求选择 `diagram_mode`。
2. 标准分层图、流程图、决策树走 `template`。
3. 复杂架构图、自由拓扑、用户明确要求“像 Visio 一样可编辑”时走 `freeform`。
...
为便于后续修改，`freeform` 模式必须要求 Agent 给每个 shape、connector、group 写稳定 `id`。后续用户说“把 API 网关移到左边”“新增一条虚线”“把这几个模块框成政务云区域”时，Agent 读取已有 `.drawio` 或保存的源模型，执行局部修改：
...
- 单元测试：freeform 输入校验、ID 唯一性、connector 引用校验、draw.io XML 生成。
