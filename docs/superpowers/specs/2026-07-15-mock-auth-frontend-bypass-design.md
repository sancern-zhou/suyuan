...
会话和资源归属切换到新用户的情况下才应覆盖它。用户 ID、用户名或显示名的空值及
纯空白覆盖不会生成无效管理员；后端分别回退到稳定 ID `local-developer`、有效用户
ID 和默认显示名。
...
配置请求最长等待 5 秒。如果请求失败、超时、响应格式错误或返回未知模式，前端
回退到 `company`，继续走现有登录流程。这是安全失败策略，不能因为配置服务异常
而意外免登录或让应用长期停留在未挂载的空白页面。

Mock 响应必须完整满足契约：顶层与用户 `sysCode` 非空且一致，用户 ID、用户名和
显示名非空，`roleCodes` 是字符串数组并包含 `SUYUAN_ADMIN`，`isAdmin=true`，且
`authSource=mock`。缺失或不匹配任一字段均按非法配置回退公司模式。
...
- 运行时配置不可用、超过 5 秒或非法：中止请求并回退公司模式。
- Mock 配置缺少完整管理员身份契约：视为非法配置并回退公司模式。
...
- Mock 用户配置为空或纯空白时回退稳定身份，不产生空所有者。
...
- 运行时配置加载失败、超时、非法或未知模式时回退公司模式；非法用例覆盖缺字段、
  角色缺失、错误身份来源和系统代码不一致。
