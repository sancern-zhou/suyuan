...
class SocialAppRepository(private val api: SocialAppApi) {
    ...
    fun stream(token: String, query: String, sessionId: String?, attachments: List<UploadedAttachment> = emptyList(), mode: String = "expert"): Flow<AgentEvent> = api.stream(token, query, sessionId, attachments, mode)
