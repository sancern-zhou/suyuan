...
class AgentAnalyzeRequest(BaseModel):
    ...
    mode: Optional[str] = Field(
        "expert",
        description="✅ Agent模式（双模式架构）：'assistant' - 助手模式（办公任务），'expert' - 专家模式（数据分析）"
    )
    ...
    **事件类型**:
    ...
    try:
        ...
        analyze_kwargs = {
            ...
            "manual_mode": request.mode  # ✅ 双模式架构：assistant/expert
        }
...
def get_expert_router_v3():
    ...
    if expert_router_v3_instance is None:
        try:
            ...
            logger.info("expert_router_v3_initialized")
        except Exception as e:
