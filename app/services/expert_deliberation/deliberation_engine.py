...
class ExpertDeliberationEngine:
    ...
    async def run_async(self, request: DeliberationRequest) -> DeliberationResult:
        ...
        experts = get_default_experts()
        analyses = await self._build_expert_analyses(request, experts, ledger, round_index=1)
        ...
        if request.options.enable_supplement_planning and request.options.max_discussion_rounds > 1:
            ...
            if new_facts:
                analyses = await self._build_expert_analyses(request, experts, ledger, round_index=2)
