...
⚠️ 重要限制：
- ❌ 不适用于编辑 Word 文档（.docx）！请使用 find_replace_word 工具
- ❌ 不适用于编辑 Word XML 文件（document.xml）！请使用 find_replace_word 工具
...
- ❌ 编辑 Word 文档（请用 find_replace_word）
        ...
        try:
            ...
            if count == 0:
                ...
                if is_word_xml:
                    ...
                    hints.insert(1, "   如果要编辑 Word 文档，应使用 find_replace_word 工具（直接操作 .docx 文件）")
                    hints.insert(2, "   如果坚持编辑 XML，必须使用 read_file(path=..., raw_mode=True) 读取原始 XML")
...
⚠️ 不适用于编辑 Word 文档！编辑 Word (.docx) 请使用 find_replace_word 工具。
