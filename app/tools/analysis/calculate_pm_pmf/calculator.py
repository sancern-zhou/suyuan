"""
PMF Calculator - 基于NIMFA无监督因子分解的源解析计算引擎
...
class PMFCalculator:
    ...
    使用无监督矩阵分解算法，从污染物组分数据中自动识别潜在污染源。
    ...
    def calculate(
        ...
        Args:
            component_data: 污染物组分数据列表
            pollutant: 目标污染物 ("PM2.5" or "PM10")
        ...
        Returns:
            {
                ...
                "timeseries": [...],             # 时间序列数据
                "performance": {...},            # 模型性能指标
                "quality_control": {...},        # 质量控制报告
                "algorithm": "nimfa",            # 使用的算法
                "summary": "..."                 # 摘要信息
            }
        """
        try:
    ...
    def _validate_data(self, component_data: List[Dict]) -> Dict[str, Any]:
        ...
        for record in component_data:
            ...
            # 支持嵌套结构
            if 'components' in standardized_record and isinstance(standardized_record['components'], dict):
    ...
    def _convert_nimfa_result(
        ...
        Args:
            nimfa_result: nimfa算法结果
            components: 组分名称列表
            timestamps: 时间戳列表
            pollutant: 污染物类型
            quality_control_report: 质量控制报告
        ...
        return {
            ...
            "timeseries": [ts.model_dump(mode='json') for ts in timeseries_converted],
            "performance": performance,
            "schema_version": "pmf.v1",
            "source_contributions": source_contributions,
            "source_concentrations": source_concentrations,
            "quality_control": quality_control_report,
            "algorithm": "nimfa",
            "nimfa_details": {
                "q_value": nimfa_result.get("q_value", 0.0),
                "evar": nimfa_result.get("evar", 0.0),
                ...
                "H_matrix": nimfa_result.get("H_matrix", [])
            },
            "factor_loadings": {
    ...
    def _merge_gas_data(
        self,
        component_data: List[Dict[str, Any]],
        gas_records: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        合并碳组分数据（OC/EC）到主组分数据

        Args:
            component_data: 水溶性离子数据列表（字典格式）
            gas_records: 碳组分数据列表（字典格式）
        ...
        for record in gas_records:
            ...
            if not timestamp:
                ...
                if isinstance(components, dict):
                    timestamp = components.get("timestamp", "")
                elif "time" in record:
                    timestamp = record.get("time", "")

            if timestamp:
                gas_dict[timestamp] = record
        ...
        for record in component_data:
            ...
            if timestamp and timestamp in gas_dict:
                gas_record = gas_dict[timestamp]

                # 从碳组分记录中提取OC/EC
                # 尝试从components中获取
                gas_components = gas_record.get("components", {})
                if isinstance(gas_components, dict):
                    for key, value in gas_components.items():
                        if key in ("OC", "EC") and isinstance(value, (int, float)):
                            merged_record["components"] = merged_record.get("components", {}).copy()
                            merged_record["components"][key] = value
                else:
                    # 扁平结构
                    for key in ("OC", "EC"):
                        if key in gas_record and isinstance(gas_record[key], (int, float)):
                            merged_record["components"] = merged_record.get("components", {}).copy()
        ...
        logger.info(
            ...
            gas_records_count=len(gas_records)
        )
