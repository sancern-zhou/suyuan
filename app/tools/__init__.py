...
def create_global_tool_registry() -> ToolRegistry:
    ...
    try:
        from app.tools.task_management import (
            create_task_tool,
            update_task_tool,
            list_tasks_tool,
            get_task_tool
        )
        registry.register(create_task_tool, priority=800)
        registry.register(update_task_tool, priority=801)
        registry.register(list_tasks_tool, priority=802)
        registry.register(get_task_tool, priority=803)
        logger.info("tool_loaded", tool="task_management_tools")
    except ImportError as e:
        logger.warning("tool_import_failed", tool="task_management_tools", error=str(e))
