...
class ReActPlanner:
    ...
    async def think_and_action_v2(
        ...
        needs_second_stage = (
            ...
            tool_name and
            tool_name not in special_tools and
    ...
    async def think_and_action_v2_streaming(
        ...
        # ✅ 关闭流式输出日志（避免控制台刷屏）
        # logger.info(f"[Planner V2 Streaming] 开始流式LLM调用，iteration={iteration}")
            ...
            async for chunk in self.llm_service.chat_streaming(messages):
                ...
                if len(buffer) > 20:
                    ...
                    try:
                        # 去除可能的思考标签
                        test_buffer = buffer
                        if "<think>" in test_buffer:
                            test_buffer = test_buffer.split("<think>")[-1]
                        if "</think>" in test_buffer:
                            test_buffer = test_buffer.split("</think>")[-1]
        ...
        if is_final_answer:
            ...
            if parsed_result.get("success") and parsed_result.get("data"):
                ...
                logger.error("[Planner V2 Streaming] 最终JSON解析失败")
                yield {
                    "type": "action",
                    "data": {
                        "thought": "解析错误",
            ...
            if not parsed_result.get("success") or not parsed_result.get("data"):
                ...
                if needs_second_stage:
                    logger.info(f"[Planner V2 Streaming] 第二阶段：参数构造，tool={tool_name}")
                    ...
                    if constructed_params:
                        action["args"] = constructed_params
                        logger.info(f"[Planner V2 Streaming] 参数构造成功")
                    else:
                        logger.error(f"[Planner V2 Streaming] 参数构造失败，tool={tool_name}")
                        yield {
                            "type": "action",
                            "data": {
                                "thought": thought,
...
"""
        ...
        # ✅ 添加完整LLM响应日志（参数构造阶段）
        logger.info(f"[Planner V2] 参数构造LLM完整响应 - length={len(llm_response)}")
        logger.debug(f"[Planner V2] 参数构造完整响应内容:\n{llm_response}")
    ...
    def plan(
        ...
        # ✅ 添加完整LLM响应日志（工具选择阶段）
        logger.info(f"[Planner] 工具选择LLM完整响应 - length={len(llm_response)}")
        logger.debug(f"[Planner] 工具选择完整响应内容:\n{llm_response}")
...
"""
    ...
    def _load_schema_and_construct_params(
        ...
        # ✅ 添加完整LLM响应日志（参数构造阶段，同步版本）
        logger.info(f"[Planner] 参数构造LLM完整响应（同步） - length={len(llm_response)}")
        logger.debug(f"[Planner] 参数构造完整响应内容（同步）:\n{llm_response}")
...
⚠️ 必须使用 schema 中明确定义的参数名，不要自己推断。
...
注意:
1. 必须使用 schema properties 中明确定义的参数名
2. 不要省略 required 参数
3. 仅输出JSON，不要额外解释
"""
        return prompt
