...
class SessionMemory:
    ...
    历史限制（参考 LobsterAI）：
    - MAX_HISTORY_TURNS = 24：最多返回 24 条消息（从最新开始）
    - 字符限制通过 max_context_tokens + 压缩机制处理
    """
    ...
    def get_messages_for_llm(self) -> List[Dict[str, Any]]:
        ...
        优化（2025-02-24）：
        - 从最新消息开始选择，保留最近 MAX_HISTORY_TURNS 条
        - 参考 LobsterAI 的简单有效方法
        - 字符限制通过 max_context_tokens + 压缩机制处理
        ...
        # ✅ 从最新开始选择消息，保留最近 MAX_HISTORY_TURNS 条
        recent_turns = self.conversation_history[-self.MAX_HISTORY_TURNS:]
